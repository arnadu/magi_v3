...
| Proactive multi-resource oversight (control-plane copilot) | [ADR-0032](docs/adr/0032-proactive-resource-oversight.md). Designed alongside ADR-0031 and likely shipped together: a daily resource report plus nine new threshold alerts (disk, LLM spend, machine upgrades, OOM, Atlas storage) with fixed default thresholds, four new copilot team skills, and a Change-Stream waker that also fixes today's gap where relayed anomalies never wake the copilot. Closes G-4 and the Atlas-storage gap. |
