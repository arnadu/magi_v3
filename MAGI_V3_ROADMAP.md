...
| 23 | ⬜ Planned | **Auth + multi-user**: Firebase Auth (Google OAuth); `userId` on missions; per-user mission scoping; one copilot session per user (`copilot-{userId}`); `userId` stamped on `llmCallLog`; per-user and admin usage endpoints; fix F-008, F-009, F-016 |
