...
| Temporary mission-machine resource upgrades + proactive resource oversight | [ADR-0031](docs/adr/0031-temporary-resource-upgrades-vm-cost.md), two parts. **Part A**: cheap-by-default machine, mission-copilot-requested temporary upgrade to a bigger tier for genuine compute bursts, bounded window with explicit renewal (never a forced mid-computation revert); a new cockpit tab reports runtime (not $) per mission, by time horizon and machine config, kept fully separate from the existing LLM-only spend cap. **Part B**: the control-plane copilot gets a daily report plus intra-day threshold alerts across VM tier, LLM cost, per-mission disk (Fly Volume, closes G-4), and MongoDB Atlas storage (closes a real, previously-incident-causing gap) — driven by its existing system prompt/mental-map/skills conventions, no new confirmation infrastructure needed. Likely closes or reframes issue #31 (suspected OOM crash-loop). |
