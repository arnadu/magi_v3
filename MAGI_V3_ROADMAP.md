...
| Temporary mission-machine resource upgrades | [ADR-0031](docs/adr/0031-temporary-resource-upgrades-vm-cost.md). Cheap-by-default machine, mission-copilot-requested temporary upgrade to a bigger tier for genuine compute bursts, bounded window with explicit renewal (never a forced mid-computation revert); a new cockpit tab reports runtime (not $) per mission, by time horizon and machine config, kept fully separate from the existing LLM-only spend cap. Likely closes or reframes issue #31 (suspected OOM crash-loop). |
