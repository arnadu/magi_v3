...
| **28g** | 🔨 **In progress** | **Resource management** — temporary mission-machine upgrades ([ADR-0031](docs/adr/0031-temporary-resource-upgrades-vm-cost.md): any agent asks, the mission-copilot decides; ≤ 60 min windows, 24 h cumulative cap with operator reset in the Limits panel, runtime-by-config cockpit tab) and proactive resource oversight ([ADR-0032](docs/adr/0032-proactive-resource-oversight.md): daily report + nine threshold alerts, Change-Stream copilot waker). Closes G-4 and the Atlas-storage gap; likely closes or reframes [#31](https://github.com/arnadu/magi_v3/issues/31). Step-by-step plan: [docs/plans/resource-management-implementation-plan.md](docs/plans/resource-management-implementation-plan.md). |
...
Currently open: **G-4** (disk monitoring, 🔴, being closed in 28g / ADR-0032), **G-5** (out-of-band alerting, 🟠,
