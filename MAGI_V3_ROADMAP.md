...
| 27 | ✅ Done | **UI consolidation — MVP milestone.** This is the sprint `MAGI_V3_ROADMAP.md`'s own "Post-MVP (after Sprint 27)" section header names as the completion line, and it's now crossed. ✅ **Cockpit reaches feature parity, `index.html` retired**: auth/login (Google + admin key), a Missions view (list/create/suspend/resume/destroy, cross-mission stats per row), a read-only template browser (templates are immutable/disk-only per ADR-0022 — browse + launch, no editor), standalone copilot chat, an in-app agent-error banner wired to the mission SSE stream, and Schedule/Log/Config tabs previously reachable only via the mission-local dashboard. The legacy `packages/control-plane/public/index.html` (1934 lines) is deleted outright, closing F-018 (unpinned CDN scripts) for free since Firebase now ships via the npm package. Two pre-existing bugs fixed along the way: `/events` SSE had no heartbeat (the real cause of reported UI timeouts on a silent WireGuard drop), and `useView()` never retried its mission fetch after a successful sign-in. ✅ **Pulled forward out of order, a live production incident**: conversation-recovery bugs + copilot model-switch guard (ADR-0025) — a model switch mid-turn raced the old turn's still-unwinding call, leaving a permanent dangling `tool_use`; the shared auto-recovery mechanism built for exactly this case had two independent bugs and never fired for any agent with real history. Fixed at the root, plus a busy guard rejecting a model switch while a turn is in flight. ✅ **PostMessage math/mermaid rendering fix + `inter-agent-comms`→`postmessage-conventions` skill rename** — `\(...\)` inline-math support added (agents kept reaching for it despite it never being documented as supported), tool description shrunk to a short pointer at the renamed skill (the same lazy-loading pattern every other platform skill uses). ✅ **Inline image embedding in chat** (port from MAGI V2) — `![alt](path)` markdown syntax against the shared workspace, same-origin-only (SSRF/tracking-pixel guard mirroring the link-rendering rule). ✅ **Control-plane copilot visibility (ADR-0027)** — a new Copilot cockpit tab with Transcripts, Files, and spend-cap Limits, none of which existed before (the copilot's own turns weren't even being recorded to `agentTurnStats`/`missionStats` until this shipped). ✅ **Touch/Pointer Events fix** — all three cockpit drag-resize handles were mouse-only, inert on iPad; switched to the Pointer Events API. ✅ **MongoDB Atlas quota incident recovered** — a 512MB M0 quota breach blocked all writes cluster-wide, cascading into a login outage via `requireAuth`'s per-request user-sync write; recovered by deleting old `llmCallLog` entries (deletes are permitted even when Atlas blocks other writes) and lowering `LOG_RETENTION_DAYS` 7→2. See [docs/implementation-history.md](docs/implementation-history.md) for full detail on each item and [ADR-0025](docs/adr/0025-conversation-recovery-and-model-switch-guard.md)/[ADR-0027](docs/adr/0027-copilot-visibility-spend-cap.md) |
| 28a | ⬜ Planned | **Reliability fixes surfaced by three weeks of live usage** (mission copilots on `gold-digest-v2` and `meteo-textbook` filed these directly). Absorbs [issue #38](https://github.com/arnadu/magi_v3/issues/38) (transient non-429 LLM errors aren't retried — one incident lost 38K tokens of generated work; extend the existing 429-retry pattern in `loop.ts`), [issue #41](https://github.com/arnadu/magi_v3/issues/41) (no operator notification on spend-cap breach — caused a real 5-day operational outage; wire a mailbox post to `["user"]` through the existing `anomalyRecorder` plumbing), [issue #37](https://github.com/arnadu/magi_v3/issues/37) (agent crash on duplicate key — non-atomic `seqInTurn` count-then-insert race in `conversation-repository.ts`; catch-and-retry), [issue #40](https://github.com/arnadu/magi_v3/issues/40) (`parseModel` wrongly assumes only `anthropic/*` models support vision — non-Anthropic OpenRouter vision models silently lose image description), [issue #30](https://github.com/arnadu/magi_v3/issues/30) (deactivated agents still shown in cockpit — missing `active` filter on the `/team` roster), and **F-024** (`ListSchedule` has no `userId` scope — a one-line fix, bumped up from `findings.md`'s stale "Backlog" priority per the audit's own objection). Also bundles [issue #25](https://github.com/arnadu/magi_v3/issues/25) (configurable per-mission VM memory, currently hardcoded 1024 MB) with [issue #31](https://github.com/arnadu/magi_v3/issues/31) (investigate suspected OOM-driven daemon crashes correlated with concurrent scheduled wakeups — needs Fly metrics to confirm root cause before sizing the fix). All small, independent patches — land before 28c's characterization tests since #30 and #41 touch `daemon.ts`/`monitor-server.ts`, the exact files 28c decomposes. |
| 28b | ⬜ Planned | **Mission-prep v1 (chat-only draft-and-launch) + beta environment for a second, trusted user.** Add a `"draft"` status to `MissionDoc` (anticipated by ADR-0021's deferred discriminated-union note); wire mission creation to the existing, already-tested inline-config branch of `POST /` without provisioning a machine; give the control-plane copilot a patch tool reusing `SaveMissionConfig`'s exact mechanics (validate, upsert-by-id agents, shallow-merge mission) gated on `status === "draft"` instead of `"suspended"`; add one explicit `ProposeAction`-confirmed "Launch" action reusing `launch_mission`'s provisioning call (`draft → provisioning → running`). No new collection, no new validation logic. Then stand up a fully separate, single-tenant beta deployment (`bash scripts/bootstrap.sh --suffix beta`, own Fly apps, own MongoDB database on the same free-tier Atlas cluster, own secrets) for the beta user — chosen over adding them to the existing deployment because CR-04's shared-secret exposure needs no privilege escalation to reach (any agent's plain `env` access gets the real `MONGODB_URI`/LLM keys), so isolation-by-separate-deployment is the safe default until CR-04's real architectural fix (Sprint 28d) lands. Sidesteps CR-04 and F-024 (already fixed in 28a anyway) entirely rather than requiring either fixed first. The structured draft-review cockpit panel (mission-prep v2 — view the draft's full agent/prompt/mental-map structure without asking the copilot to read it back) is an explicit fast-follow, not a precondition for beta access. |
| 28c | ⬜ Planned | **Structural decomposition of `monitor-server.ts`/`daemon.ts` + file-scoped security fixes.** Renumbered from 28a on 2026-09-05 to make room for the reliability-fix/mission-prep/beta work above, which took priority — no content change. Split out of a single Sprint 28 following the [2026-08-09 independent audit](docs/code-review-audit-report-2026-08-09.md) (response: `docs/code-review-audit-response-2026-08-12.md`), which flagged both files as god-function-sized (`MonitorServer.handleRequest` ~725 lines; `daemon.ts`'s `main()` ~745 lines) and hard to review safely. Order: (1) characterization/integration test coverage for `handleRequest` and `main()`'s bootstrap sequence first (extends the existing `dashboard.integration.test.ts` `MonitorServer` coverage) — a regression safety net before touching either function; (2) split `handleRequest` into a route table + per-route handler modules; (3) split `main()` into named bootstrap-phase functions; (4) land CR-01 (root escalation), CR-02 (shell-interpolated agent ID), and CR-05 (auth token handling) inside the newly decomposed structure, so these two files are touched once rather than refactored twice. Also ships `docs/code-structure.md` (oversized-file inventory) and the `/code-structure-review` command (new CLAUDE.md "Code Structure" section, sprint-close check). Sequenced ahead of 28d because CR-01/02/05 already require editing these exact two files. |
| 28d | ⬜ Planned | **Remaining operational + security hardening.** Renumbered from 28b on 2026-09-05 — no content change. G-5 out-of-band alerting (webhook/email on `agent-error`) — absorbs [issue #3](https://github.com/arnadu/magi_v3/issues/3) (background job failures not surfaced) and [issue #4](https://github.com/arnadu/magi_v3/issues/4) (Change Stream reconnect silent), both the same "push a transient SSE status/alert event on a known failure" pattern G-5 already covers; **G-4 disk monitoring** (log Fly Volume usage in the daemon heartbeat, surface in the dashboard stats tab — pulled in here as the highest-severity 🔴 unscheduled operational gap, and likely shares the same dashboard-alert plumbing as G-5); onboarding flow (first-login wizard); usage dashboard (per-user spend history); the rest of the 2026-08-09 audit's `/security-review` findings — CR-03 (BrowseWeb SSRF egress), **CR-04 (shared mission secrets — the real architectural fix: per-mission/per-tenant least-privilege credentials, not just the 28b isolation workaround)**, CR-06 (CI/CD supply-chain gates), CR-07 (external-action confirmation), CR-08 (sensitive-data posture) — absorbs [issue #7](https://github.com/arnadu/magi_v3/issues/7) (Firebase `checkRevoked`) and [issue #21](https://github.com/arnadu/magi_v3/issues/21) (protect `sharedDir`'s `.git` history from destruction — substantial enough for its own line item, not folded silently into the generic review); deployment documentation update. **Also in scope: unblock F-021/F-023/F-026** — these were parked on "no confirmable UI surface yet"; the cockpit's chat drawer (Sprint 26b) is that surface now. Independent of 28c's file changes; sequenced after because 28c is the harder/riskier piece with an internal dependency on the same files. |
...
| ~~**G-3**~~ ~~Missed cron fires not replayed on daemon restart~~ | ~~🔴 Data loss~~ | **Corrected/closed** — see `docs/operational-resilience.md`'s Gap summary (2026-07-22 correction; narrower real gap closed Sprint 26c via `MAX_DELIVERY_ATTEMPTS`) | ✅ |
| ~~**G-6**~~ ~~Orphaned background jobs not cleaned on restart~~ | ~~🟠 Mission stall~~ | **Closed Sprint 12** — `recoverOrphanedJobs()` in `daemon.ts` | ✅ |
| **G-4** No disk monitoring for Fly Volume — volume fills silently, writes fail with no alert | 🔴 Data loss | Moderate — log disk usage in daemon heartbeat; surface in dashboard stats tab | **28d** |
...
| **G-5** No out-of-band alerting for LLM auth failure / credits exhausted — operator must notice dashboard banner | 🟠 Mission stall | Moderate — POST to a webhook or send email on `agent-error` with `transient: false` | **28d** |
