...
| Temporary mission-machine resource upgrades | [ADR-0031](docs/adr/0031-temporary-resource-upgrades-vm-cost.md). Cheap-by-default machine; any agent can ask via `PostMessage`, but only the mission-copilot decides and requests the actual upgrade to a bigger tier for genuine compute bursts — a bounded window with renewal, both the copilot and the requesting agent reminded near expiry; a new cockpit tab reports runtime (not $) per mission, by time horizon and machine config, kept fully separate from the existing LLM-only spend cap. Likely closes or reframes issue #31 (suspected OOM crash-loop). |
| Proactive multi-resource oversight (control-plane copilot) | [ADR-0032](docs/adr/0032-proactive-resource-oversight.md). Designed alongside ADR-0031 and likely shipped together, but a separate concern: a daily report plus intra-day threshold alerts across VM tier, LLM cost, per-mission disk (Fly Volume, closes G-4), and MongoDB Atlas storage (closes a real, previously-incident-causing gap) — driven by the control-plane copilot's existing system prompt/mental-map/skills conventions, no new confirmation infrastructure needed. |
