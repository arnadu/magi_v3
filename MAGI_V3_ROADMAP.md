...
| Temporary mission-machine resource upgrades + machine-runtime reporting | [ADR-0031](docs/adr/0031-temporary-resource-upgrades-vm-cost.md). Cheap-by-default machine, mission-copilot-requested temporary upgrade to a bigger tier (e.g. `performance-1x`) for genuine compute bursts, bounded window with explicit renewal (never a forced mid-computation revert), relayed to the owning user's control-plane copilot via the existing anomaly pipeline. Deliberately does *not* fold machine cost into the existing $ spend cap — a new cockpit tab reports runtime (not $) per mission, by time horizon and machine config, kept fully separate from LLM cost accounting. Likely closes or reframes issue #31 (suspected OOM crash-loop). |
