...
| 26c | ⬜ Planned | **Close out the 24–26 alignment-infrastructure arc.** Cockpit leftovers from 26b: Files panel direct-edit (write side, notify-last-agent), cockpit-vs-chat mode auto-selection, copilot wake-up attribution + persisted anomaly/limit-breach logging (new backend instrumentation — no data source exists today). Plus the natural conclusion of this window's single-source-of-truth thread: objectives → MongoDB migration, removing the Fly-volume-vs-`teamFiles` two-copy architecture entirely (tools replace the Bash-script skill, since agent Bash subprocesses have no `MONGODB_URI` by design) — see [ADR-0019](docs/adr/0019-objectives-mongodb-migration.md) and [issue #23](https://github.com/arnadu/magi_v3/issues/23) for the full design and cost breakdown. Kept separate from Sprint 27 deliberately: 27 is genuinely different in kind (external launch readiness — alerting, onboarding, security, UI consolidation), and folding an internal data-model migration into it would dilute that focus |
