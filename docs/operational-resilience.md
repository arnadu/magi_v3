...
| Fixed the actual root cause behind the Atlas quota outage (below) recurring a second time within 24 hours: `llmCallLog.input.messages` was storing the entire accumulated conversation on every logged call (O(n²) growth over a mission's life), not just what changed since the last one — at the second incident 92% of all body-carrying data was under 6 hours old, so the retention-window-only mitigation from the first incident had almost no effect. Fixed at the source (`truncateOldMessages()` in `llm-call-log.ts`, caps each entry to the most recent 40 messages); the pruner (`scheduler.ts`) also switched from `$unset`-stripping to full `deleteMany` (proven to still work even when Atlas blocks other writes, unlike updates) and now runs every 30 minutes instead of daily, with `LOG_RETENTION_DAYS` lowered 2→1 | 27 (unplanned) | Closes the "still no Atlas storage monitoring" gap's practical impact (not the gap itself, which remains open) — the mechanism that made quota exhaustion happen in *hours* rather than *weeks* is gone, and the pruner can no longer be blocked by the exact condition it exists to prevent |
...
mitigated:** `llmCallLog.input.messages` stored the *entire accumulated conversation* on every
single logged call — not just what was new since the previous one. On a long-running mission,
call *N* re-stores everything calls *1* through *N-1* already stored: O(n²) growth over a
mission's life. This is why narrowing the retention window alone didn't hold: at the second
incident, 92% of all body-carrying data was under 6 hours old, so shortening the window further
had almost no effect — the generation *rate*, not the retention *age*, was the actual problem.
Fixed at the source in `llm-call-log.ts`'s `truncateOldMessages()`: only the most recent 40
messages are kept per logged call (mirrors the existing `truncateToolBodies` pattern), bounding
each entry's size to roughly constant regardless of mission depth. Older messages remain fully
recoverable from `conversationMessages`, the authoritative store this was always redundant with.
