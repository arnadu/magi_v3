...
| `mission.volumeId` in MongoDB diverges from the real Fly volume | A resume deletes the mission's working machine, then fails to re-create it against the (wrong) recorded volume — the mission goes down even though the real data volume is untouched | 🟠 | `volumeExists()` check in the resume handler (`missions.ts`), added after a live incident (2026-09-23, issue [#62](https://github.com/arnadu/magi_v3/issues/62)) — refuses to delete the current machine unless the target volume is confirmed to exist first. **Not a one-off**: a proactive sweep of every mission's `volumeId` against the real Fly volumes list found a second, not-yet-triggered instance (`test2`, still suspended) with the same symptom — corrected directly, no incident occurred | The mechanism that lets `volumeId` drift in the first place is still unknown — every in-app write path was checked and ruled out. Two occurrences across unrelated missions rules out a one-time manual mistake; a race between two missions' provision responses is one unconfirmed lead (see issue #62). Resume failures via this route also raise no `AnomalyRecorder` entry, unlike ADR-0031's `resize-failure` — a silent failure mode until someone notices the mission's status, which is exactly how this drifted undetected. **Detection added**: `reconcile-mission-state.mjs`'s report-only mode now flags any mission whose `volumeId` doesn't match a real Fly volume (2026-09-24) — run it periodically until the drift mechanism itself is found. The control-plane copilot still has no way to check this on its own (no live volume-existence tool; it inferred "volume destroyed" wrongly from error text during the incident) — tracked for a follow-up, not yet built |
