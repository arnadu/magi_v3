...
| `mission.volumeId` in MongoDB diverges from the real Fly volume | A resume deletes the mission's working machine, then fails to re-create it against the (wrong) recorded volume — the mission goes down even though the real data volume is untouched | 🟠 | `volumeExists()` check in the resume handler (`missions.ts`), added after a live incident (2026-09-23, issue [#62](https://github.com/arnadu/magi_v3/issues/62)) — refuses to delete the current machine unless the target volume is confirmed to exist first | The mechanism that lets `volumeId` drift in the first place is still unknown — every in-app write path was checked and ruled out; likely a manual/direct MongoDB edit. Resume failures via this route also raise no `AnomalyRecorder` entry, unlike ADR-0031's `resize-failure` — a silent failure mode until someone notices the mission's status |
