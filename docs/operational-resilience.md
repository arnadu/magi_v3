...
and `output` from `llmCallLog` entries older than `LOG_RETENTION_DAYS` (2, lowered from 7 on
2026-08-09 — see incident below), keeping usage/cost metadata indefinitely. The pruner also runs
once on control-plane startup to catch any entries missed during downtime.
...
| Atlas write-blocked (quota exceeded) | `$unset` fails; log cannot be pruned; **confirmed live, 2026-08-09: Atlas blocks writes cluster-wide once truly at quota, including the per-request `syncFirebaseUser` upsert in `requireAuth` — this took login down, not just pruning** | 🔴 | Deletes are still permitted when updates are blocked (confirmed live); recovered by deleting the oldest full-body `llmCallLog` entries directly (792 docs, ~163MB) rather than the usual strip-only prune, then lowering `LOG_RETENTION_DAYS` 7→2 so less full-body data is ever "in flight" at once | Still no monitoring/alerting on Atlas storage — the failure mode is now understood and the retention window narrowed, but a high enough sustained call volume can still exhaust a 512MB M0 tier before the 2-day window prunes it; durable fix is Atlas storage alerting or upgrading tier, neither done yet |
...
| Pruning removes `input`/`output` needed for active debugging | Cannot reconstruct exact LLM call context for old entries | 🟡 | 2-day window covers the most active debugging scenarios; `usage` metadata always retained | Narrower than before (was 7 days) — extend `LOG_RETENTION_DAYS` if 2 days proves too short in practice |
