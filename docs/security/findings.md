...
| F-025 | LOW | Backlog | `packages/agent-runtime-worker/src/mission-copilot-tools.ts` (`SetMissionSpendCap`), `monitor-server.ts` (`/set-budget`) | **Mission copilot can raise its own mission's hard spend cap unconfirmed, and as of ADR-0018 that change is now durable, not ephemeral** — `SetMissionSpendCap` posts an audit-trail mailbox message but has no confirmation gate, and the cap it raises also funds the copilot's own further LLM calls (self-referential). Before ADR-0018 (2026-07-21), `/set-budget` only mutated `MonitorServer`'s in-memory `currentCapUsd` — a copilot-raised cap was lost on the next daemon restart. `/set-budget` now persists the new cap to the same MongoDB document (`missions.teamConfigYaml`) an operator's own cockpit edit writes to, so a copilot-raised cap survives indefinitely, same as an operator-set one, until explicitly changed again. Two options recorded, neither decided in code: (a) accept as the same trust tier as any other unconfirmed mission-agent action, or (b) cap the increase per call (e.g. 2× original) without confirmation. | Decide (a) vs (b) explicitly once real usage data exists; do not resolve silently by picking one in a future refactor without updating this row. The durability change is a reasonable side effect of fixing the underlying single-source-of-truth bug (ADR-0018), not something to revert — but it does mean this finding's practical impact is now slightly larger than "unconfirmed for one session." |
