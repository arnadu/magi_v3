...
| F-008 | LOW | Pre-prod | `monitor-server.ts` | **Unauthenticated control endpoints (defence-in-depth gap)** — `POST /stop`, `POST /send-message`, `POST /extend-budget` have no auth on the MonitorServer itself. In production, port 4000 is only reachable via the control plane proxy (TB-11), which now scopes access to the mission owner (F-019 fixed). The risk is limited to compromise of another machine in the same Fly.io WireGuard org — a high bar. In local dev, port 4000 is on localhost and only reachable by the developer. | Add `MONITOR_TOKEN` env var to the machine at provision time; proxy forwards it as `X-Monitor-Token`; MonitorServer checks it on mutating routes. Skip check if `MONITOR_TOKEN` is absent (dev mode). |
