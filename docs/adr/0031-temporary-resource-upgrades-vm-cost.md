...
avoiding a bad-timing suspend onto the mission-copilot's own judgment (see Decision 3), not new
platform orchestration.

**`SetMissionSpendCap`'s exact shape is the concrete template for the new tool** (verified,
`mission-copilot-tools.ts:745-761`): a single-parameter `Type.Object` (`capUsd: Type.Number(...)`),
an `execute()` that calls a route and then posts an audit message via a shared `auditPost(subject,
body)` helper (`:198-206`) that always posts `{ from: MISSION_COPILOT_AGENT_ID, to: ["user"] }` into
that mission's own mailbox — the exact same helper every other mutating mission-copilot tool uses.
But `SetMissionSpendCap` calls its own mission's loopback monitor server
...
the `missionId` in the request body/query on its own. No new auth mechanism needed; just a new route
under this same middleware.
...
**2. Concrete tool signature**, modeled directly on `SetMissionSpendCap`:
...
const requestResourceUpgrade: MagiTool = {
  ...
  description:
    ...
    "(e.g. a large in-memory data transform). Interrupts any currently-running agent turns " +
    "and background jobs on this mission (same as a manual suspend) — do not call this while " +
    "a job you need to complete is running; wait for it, or accept it will be retried from " +
    "scratch. Must be renewed before it expires or the mission reverts to its default tier.",
...
`provisionMission(existingVolumeId, ...)` sequence `resumeMission()` already established.
...
mission-copilot is responsible for judging *when* to call this tool and *when* to let a renewal
lapse — via prompt/skill guidance (a natural fit for ADR-0032's `resource-oversight` skill,
"don't request an upgrade while a job you care about is running") — not a platform-enforced
idle-wait. This was deliberately chosen over building new orchestration (exporting a live
"is this mission idle" signal, polling for `runningJobs === 0`, bounding a wait with a timeout) —
that mechanism was explored and rejected as unnecessary complexity once the copilot itself is
expected to manage timing; see Alternatives.

**4. Notifications, three distinct things, not two:**
- **Mission-cockpit visibility** — the request/renewal posts a mission-scoped audit message via the
  same `auditPost` helper `SetMissionSpendCap` already uses (`from: MISSION_COPILOT_AGENT_ID, to:
  ["user"]`), visible in that mission's own Conversations panel.
...
- **Wait for the mission to be genuinely idle before resizing** (no in-flight agent dispatch, no
  ...
  More importantly, it doesn't actually solve the motivating scenario: if the copilot requests an
  ...
  favor of Decision 3 — the copilot manages timing itself, and the tool is documented as something
  to call *before* starting known-heavy work, not as a mid-crisis rescue.
- **Fully open-ended upgrade, no expiry at all.** Rejected: unbounded runtime-at-expensive-tier
  ...
  because it gives an explicit, audited checkpoint ("do I still need this") the copilot already has
  to reason about anyway when managing its own job timing around suspends.
...
- **Confirmation requirement — now more open than before, not less.** Because machine runtime is
  ...
  **no dollar ceiling bounding it at all** — a materially different risk profile than
  `SetMissionSpendCap`, which at least answers to the #49 ceiling. This is sharper still now that
  Decision 4 makes the routine case silent (mission-cockpit audit message + data logging only, no
  live relay to the control-plane copilot) — nothing short of ADR-0032's own exceptional-pattern
  alert would ever flag a mission stuck renewing an expensive tier indefinitely. Candidates: (a)
  a dedicated, operator-settable ceiling on cumulative upgraded-tier runtime or renewal count,
  mirroring the #49 ceiling pattern but denominated in time, not dollars — smaller lift than full
  `ProposeAction` infra (which the mission-copilot doesn't have at all today — same gap CR-07/Sprint
  28f tracks); (b) require confirmation on the *first* request only, not each renewal; (c) rely
  entirely on ADR-0032's exceptional-pattern alert as the only backstop, with no dedicated ceiling.
  Leaning toward (a) as the pragmatic default, but this needs a decision before implementation, not
  an assumption.
...
- No new orchestration for "safe" suspend timing — deliberately simpler than an earlier draft
  ...
  instead, which needs explicit prompt/skill guidance (ADR-0032's `resource-oversight` skill), not
  just code.
