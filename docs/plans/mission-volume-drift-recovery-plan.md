...
| 6 | ✅ **Close-out.** `docs/operational-resilience.md`'s gap row updated (severity 🟠 → 🟡; documents what the copilot can now do, narrows the open gap to just the drift mechanism itself); `MAGI_V3_SPEC.md`'s `GetMissionStatus` row, `AnomalyCategory` count/list (nine → ten, `resume-failure`), and a new paragraph on `fix_mission_volume`/`resume_mission`'s fallback; full regression (`npm run lint && npm test`, 933/933 passing). **Not live-verified**: the user declined a live check (it would have meant creating a throwaway mission and manually drifting its `volumeId` in the real dev MongoDB/Fly environment) — all logic is covered by unit/integration tests with mocked Fly calls only, per this project's own honesty convention around live checks. | `docs/operational-resilience.md`, `MAGI_V3_SPEC.md` | — |

**Plan complete.** All 6 steps shipped and pushed to `main` (commits `ff243ff`, `2c369c9`, `e83f20b`, `6df37bc`, plus this close-out commit). Root cause of the drift itself remains open — issue #62 stays open for that.
