...
| 3.1 Reset route + read data | ✅ 2026-09-22: `writeUpgradedRuntimeReset` (missions.ts, mirrors `writeMissionCostCeiling`'s F-025 separation, exports the existing `postLimitsAudit` for reuse) behind `PATCH /:id/limits/upgrade-reset`; `GET /:id/limits`'s `upgrades: {usedHours, capHours, resetAt, active}` block; new `mission-resource-routes.ts` → `GET /:id/machine-runtime` (runtime by config, four horizons). 10 integration tests against real Mongo (cross-user 404s on all three, reset zeroes `usedHours` without touching segment history, horizon windowing) |
| 3.2 Limits panel section | ✅ 2026-09-22: `UpgradeLimitsSection.tsx` (used/cap bar reusing `LimitsPanel`'s `pctColor`/`Minibar`, active-upgrade line, two-step inline Reset confirmation — not a bare one-click button, per the documented Destroy-button incident, 2026-09-11), wired into `LimitsPanel.tsx`; `resetUpgradedRuntime` + `upgrades` type in `data.ts`. 14 unit tests for the pure formatters. **LIVE browser check** (Playwright against the real control plane run locally on the real dev Mongo, admin-key login, `zz-lifecycle-smoke-f5nlew`): confirmed the golden path (used/cap bar, active-upgrade line, Reset → Confirm/Cancel, real reset call updates the bar and posts the audit message already visible in Conversations) — and, doing so, found and fixed a real pre-existing bug in `LimitsPanel.tsx`: every section's post-save confirmation note (mine, and the existing spend-cap/ceiling ones) was wiped almost instantly by the panel's own full-remount-on-refresh, because `setData(null)` ran on every refresh, not just on a mission switch. Fixed by tracking the last-seen `missionId` and only blanking the panel on a genuine switch. **Not fixed, and out of scope for this step**: at phone width the whole mission-detail view (sidebar + main columns) overflows horizontally — `styles.css` has zero `@media` queries anywhere in the app, so this is a pre-existing, whole-cockpit gap affecting every panel identically, not something this new section introduces; filed as issue [#57](https://github.com/arnadu/magi_v3/issues/57) |
| 3.3 Runtime tab | ✅ 2026-09-22: new `Runtime` main tab (`RuntimePanel.tsx`) showing wall-clock machine time by config across four horizons (today/7d/30d/lifetime), reusing `GET /:id/machine-runtime` from step 3.1, now extended server-side with a display-only `estimatedCostUsd` per row (`estimateCostPerHourUsd` from `machine-shapes.ts` — computed once on the server, not duplicated client-side). 8 unit tests for the pure formatters. **LIVE browser check** against real accumulated history on `zz-lifecycle-smoke-f5nlew` (real prior upgrade segments from earlier steps, plus two freshly seeded rows): confirmed all four horizons render, correct per-horizon inclusion/exclusion (today excludes a 20h-old segment that 7d/30d/lifetime correctly include), the upgraded badge, the cost estimate, and Refresh — all matching the existing card/badge visual style with no new styling issues |
| 4.1–4.2 `request-resources` skill | ✅ 2026-09-22: `packages/skills/request-resources/SKILL.md` (worker-agent and mission-copilot sections, matching ADR-0031 Decision 3 verbatim; the price/shape menu is checked against `buildMenu()` by test, not hand-maintained); one-line pointer added to `run-background/SKILL.md`; confirmed the mission-copilot's `disabledSkills` does not exclude it (`mission-copilot.ts:352-358`) — no code change needed. 14 tests: platform-tier discovery, both sections present, real tool names referenced, the `MISSION_COPILOT_ENABLED=false` fallback line, and (mutation-checked) the menu table matches `buildMenu()` shape-for-shape and price-for-price. `MAGI_V3_SPEC.md`'s platform-skills list updated (the Tier B tool row was already added in step 2.5). **Content — wording, not mechanics — is yours to review** |
| 5.2 Atlas usage | ✅ 2026-09-22: `atlas-usage.ts` (`readAtlasUsage`, `checkAtlasStorage`): `listDatabases` → sum `dataSize + indexSize` per database (never `storageSize`) over every non-system database, falls back to the app database alone with a logged warning if listing is denied; `$collStats` breakdown (top 10) for the app database; upserts `platformResources` `_id: "atlas"`; relays `atlas-storage-high` to `platformAdminUserIds` (parsed by the caller — wired in step 5.4) only at the hard threshold, soft is report-only; warns (every tick, not once) when no admins are configured. 13 unit tests over a fully faked cluster (mutation-checked: `dataSize+indexSize` vs `storageSize`, and the soft/hard relay split). `platformResources` documented in `CLAUDE.md`; new operational-resilience rows; added gap **G-9** (`conversationMessages` unbounded growth — the real driver, confirmed live 2026-09-20) per the plan's own commitment. **LIVE 2026-09-23**: rather than a manual Atlas-UI screenshot comparison (not practical from an agent session — no logged-in browser), independently recomputed the exact same `dataSize + indexSize` methodology directly against the live dev cluster via raw `dbStats` calls over `flyctl ssh console`. Matched the code's own persisted `platformResources` figure closely (397.4 MB stored vs 407.2 MB independently computed moments later — the ~10 MB gap is real, expected growth from live activity, including this same test's own writes, in the few minutes between samples). Confirmed independently: exactly the same 11 non-system databases counted (all of `admin`/`local`/`config` correctly excluded), `magi_v3_dev`'s `dataSize` (359 MB) is far above its `storageSize` (171 MB) exactly as the ADR's own prior finding describes, and `conversationMessages` (323 MB) remains the dominant driver — this is at least as rigorous a check as a UI screenshot, since it re-derives the number from the same raw data by an independent path rather than trusting a second rendering of the same stored figure |
| 5.1 Disk sampler | ✅ 2026-09-22: `resource-sampler.ts` (`sampleDiskUsage`, injected `statfs`/`du`/alert store), called from `daemon.ts`'s existing 60 s job-runner tick next to `logMemoryUsage`; upserts `missionResources` with `diskUsedBytes/diskTotalBytes/rssMb/runningJobs`; raises `disk-usage-high` (soft 80% / hard 90%) with a bounded `du -x -k --max-depth=3` top-5-directory breakdown, gated on `FLY_APP_NAME` being set (skipped in local dev). Threaded `db`/`anomalyRecorder` through `startJobRunner`/`startBackgroundJobs` (`BootContext` gains no new field — reuses `db`/`anomalyRecorder` already there). 15 unit tests (parsing incl. permission-error lines, both thresholds, du/statfs/Mongo/store/recorder failures never break the tick, fail-open matches step 1.2's own design). Closes **G-4**. **LIVE 2026-09-23**, on `magi-missions-dev` (`zz-lifecycle-smoke-f5nlew`, resumed for this test, resuspended after): `fallocate`d a dummy file on the mission's real 10 GB Fly volume to 87% — the very next 60 s sampler tick raised a real `disk-usage-high` **soft** anomaly with the exact percentage and byte figures, plus a correct bounded top-5-directory breakdown (`/missions`, `/missions/missions`, the mission's own dir, its `shared` subdir, `/missions/home`, largest first). Grew the file to 91% — the next tick raised a real **hard** anomaly, same breakdown shape, correctly escalated. Freed the space and confirmed the filesystem returned to 1% via `df`. **Aside, unrelated to this work**: this same throwaway mission's `mission-copilot` is stuck in a genuine runaway retry loop (repeated `RequestResourceUpgrade` retries hitting an OpenRouter 402 in-flight-budget error, ~50 dispatches per resume, each burning real API cost before the daemon self-exits within ~2 min) — pre-existing, first surfaced during this same session's step 2.7 live test; resuming the mission to run this check re-triggered it three times. Already tracked: issue [#56](https://github.com/arnadu/magi_v3/issues/56) (the exact "unsaved tool result traps mission-copilot in synthetic-result loop" mechanism observed here) and [#55](https://github.com/arnadu/magi_v3/issues/55) (the 402-driven tight retry pattern); issue [#58](https://github.com/arnadu/magi_v3/issues/58), filed minutes before this test by a copilot noticing the resulting anomaly flood, is a further symptom of the same root cause. No new issue needed — checked via `ListIssues`-equivalent search before considering filing one. Left the mission suspended |
| 5.3 OOM detection | ✅ 2026-09-22: `fly-events.ts` (`classifyExit`, `checkOomExits`) plus a new `listMachines()` in `fly-machines.ts` (`GET /apps/{app}/machines`, empty in local execution). `classifyExit` checks `requested_stop` first (never OOM, even if the exit code also looks OOM-like), then `oom_killed === true \| signal === 9 \| guest_signal === 9 \| exit_code === 137`. Dedup reuses `AlertStateStore` generically as a plain existence check on `${machineId}:oom-suspected:${exitedAt}` (not `evaluateAlert`'s ratio logic — this is a discrete one-time event); an event with no `exited_at` is skipped rather than risking a repeat report forever. Raises `oom-suspected` (hard) via a mission-scoped `AnomalyRecorder` built the same way `resource-upgrade.ts`'s `recordAnomaly()` does, looked up from `missions` by `machineId`; an orphaned machine (mission already destroyed/re-provisioned) logs and skips rather than throwing. 19 unit tests: real dev-captured fixtures (requested stop, `exit_code: 1` restart) from 2026-09-20 plus synthetic 137/signal-9/`oom_killed` cases; no double-report across ticks; a later exit on the same machine still reports; fail-open on a broken dedup store; `listMachines` failure never throws. **LIVE: none available.** The plan originally expected this to piggyback on step 2.7's live test, but 2.7's `MemoryError` was a clean Python exception inside the container, never a Fly-level machine kill — no real `oom_killed`/137/signal-9 exit event was produced. This module is therefore unverified against a real Fly OOM exit; flagging rather than claiming coverage that doesn't exist. Wiring into the 5-min tick (so this actually runs) is step 5.4 |
| 5.4 Resource monitor tick | ✅ 2026-09-22: `resource-alerts.ts` (pure ratio calculators: `spendCapRatio`, `spendSpikeRatio`, `upgradeCapNearRatio`, `upgradeIdleRatio`) and `resource-monitor.ts` (`runResourceMonitorTick`/`startResourceMonitor`, `node-cron` `*/5 * * * *`, runs once immediately on startup, started from `index.ts` alongside the scheduler and copilot waker). Per running mission: `spend-cap-near` (soft 90%/hard 98% of `mission.maxCostUsd`, from `readMissionSnapshot` — lifetime + any in-flight turn, never cached); `spend-spike` (soft only, 24h `agentTurnStats.costUsd` vs 3x the trailing 7-day daily average and >$5, using the `{missionId, startedAt}` index built for exactly this in step 1.4); `upgrade-cap-near` (soft only, 80% of the 24h cumulative cap via `upgradedMsSince`, respecting `upgradedRuntimeResetAt`); `upgrade-idle` (soft only, upgraded + 30 min with no running job from the disk sampler's `missionResources.runningJobs` reading + no conversation activity from `missionStats.lastTurnAt`, both a missing sample and a missing activity timestamp treated as "unknown," never as "idle"). The three soft-only categories use new `LevelThresholds` pairs (`UPGRADE_CAP_NEAR`, `UPGRADE_IDLE`, `SPEND_SPIKE_RATIO`, all `hard: Infinity`) so they reuse `evaluateAlert`'s existing dedup/hysteresis rather than new logic. Also invokes 5.2's `checkAtlasStorage` and 5.3's `checkOomExits` on the same tick — every check (cross-cutting and per-mission) is independently caught so one failure never stops the rest. Refactored the repeated "mission's own mailbox + `copilot-{userId}` relay" `AnomalyRecorder` wiring (previously duplicated in `resource-upgrade.ts` and `fly-events.ts`) into a shared `createMongoAnomalyRecorderForMission(db, missionId, userId)` in `anomaly.ts`, used by all three call sites now. New env vars `PLATFORM_ADMIN_USER_IDS` and `ATLAS_STORAGE_LIMIT_MB` parsed in `index.ts` and documented in `CLAUDE.md`. 15 tests for the pure ratio calculators, 19 for the tick's orchestration (including the F-028 regression: two missions with different owners never cross-contaminate their `copilot-{userId}` mailboxes) and 2 new tests for the shared anomaly-recorder helper; extended the existing `resource-alert-state.unit.test.ts` threshold sanity checks to cover the three new soft-only constants. Extended the shared `fake-db.ts` test helper with minimal `$match`/`$group: {$sum}` aggregation support (only what this windowed query needs, documented as such). New operational-resilience section for the tick itself. **LIVE: none** — this needs a real running control plane against real accumulating `agentTurnStats`/`machineSegments`/`missionResources` data over hours-to-days to observe a genuine threshold crossing; not practical to stage in this session, flagging rather than claiming it |
| 6.1 Daily report | ✅ 2026-09-22: `resource-report.ts` (`buildDailyReport`, `renderDailyReport`, `runDailyReportsIfDue`), plus a new shared `resource-queries.ts` (`sumTurnCostUsd`, `lastActivityAt`, `latestResourceSample` — extracted out of `resource-monitor.ts`, now used by both). One report per user owning a non-destroyed, non-draft mission, wired into the existing 5-min tick (`resource-monitor.ts`) rather than a separate cron, firing once `now` is at or past `RESOURCE_REPORT_HOUR_UTC` (default 12) for the day — a genuine catch-up, not just an exact-hour match, so a control plane down through the whole report hour still sends later the same day. Idempotency is an atomic `$setOnInsert` upsert on `resourceSnapshots` `(userId, date)`: only the caller whose `upsertedCount` is 1 posts the mailbox message, so a race or retry never double-sends; persist-before-post trades a rare "claimed but undelivered" failure (logged) for never double-posting on a crash between the two steps. Flags computed: Atlas ≥ 70% (admins only, from the already-persisted `platformResources` doc, no extra Atlas call), per-mission disk ≥ 80% or projected full within 7 days (from yesterday's `resourceSnapshots` growth rate), spend ≥ 80% of cap, upgrade-cap-near ≥ 80%, upgraded-idle (reusing `upgradeIdleRatio`), monitoring-blind for a `running` mission with no or stale (>5 min) sample, chronic upgrade (≥ 5 of the last 7 `resourceSnapshots` show `upgradedMs > 0`), and any `upgrade-cap-reached` anomaly in the last 24h. **Deliberately scoped down from the ADR's illustrative sample**: the sample's "N requests, M renewals" upgrade counts aren't derivable from the current `machineSegments` schema (a same-shape renewal moves `plannedEndAt` rather than opening a new segment, so there's no stored renewal count) — this report shows total upgraded time and any currently-active upgrade instead, rather than inventing a new counter mid-implementation; noted in the module's own header comment. 29 new tests (`resource-report.unit.test.ts`): per-flag table-driven cases on both sides of each threshold, suspended and never-sampled missions, Atlas block admin-gating, multi-user isolation (F-028 regression: each user's report only ever mentions their own missions), idempotency on a same-day second run, catch-up hours after a missed report hour, a custom `reportHourUtc`, and one user's build failure never blocking another's send. Plus 6 tests for the extracted `resource-queries.ts`. Extended `fake-db.ts` with `distinct`, `find().limit()`, and `$nin` support (all newly needed here, none previously exercised). New env var `RESOURCE_REPORT_HOUR_UTC` documented in `CLAUDE.md`; `resourceSnapshots` added to the storage list; new operational-resilience section. **LIVE: none** — needs real multi-day accumulated data to see a genuine growth-rate/chronic-upgrade flag; deferred to step 6.3's live check once the copilot side (6.2) also exists to receive it |
| 6.2 Copilot behaviour | ✅ 2026-09-22: `config/teams/copilot.yaml` gets a "Resource oversight" system-prompt section (what the daily report and the four new hard-relayed category groups are, the one-line all-clear rule, ≤120-word digest with at most one proposed next step) and a matching "Resource oversight" mental-map table, same convention as the existing Anomaly log. Four new team skills in `config/teams/copilot/skills/`: `daily-resource-report` (read order: flags → already-flagged check → digest → mental-map update → cross-mission pattern check), `disk-pressure` (route by what's growing: logs/temp → ask the mission's copilot to prune; agent data → tell the operator, no `extend_volume` action type exists; ≥95%/full-within-2h → `suspend_mission`), `atlas-storage` (read-only — breakdown → ordered levers: drop unused databases, a not-yet-built retention rule, an Atlas tier upgrade), `vm-upgrade-oversight` (`oom-suspected`/idle/cap-near/cap-reached/chronic, one subsection each). Extended `cost-management` with `spend-cap-near`/`spend-spike` sections — and, while there, corrected its pre-existing "Spend stats fields"/"Cost attribution" sections, which pointed at `GET /api/missions/stats`/`llmCallLog` (unreliable per the ADR's own finding, issue #53) to instead use `missionStats`/`agentTurnStats`, the same collections the new alerts and the report actually read, so the skill no longer contradicts itself. Extended `mission-recovery` with a `resize-failure` failure-mode entry (propose `resume_mission`; a stale `machineId` is already tolerated by that route's own best-effort delete). Extended the platform `incident-triage` skill (shared with the mission copilot) with one dual-audience section per new category, naming each control-plane-copilot skill above for that audience while giving the mission copilot enough to act with its own tools (`PauseAgent`, `SetMissionSpendCap`, `RequestResourceUpgrade`, `EndResourceUpgrade`) directly, since the four new skills are team-scoped to the control-plane copilot only (deliberately, per the ADR, to avoid bloating every mission agent's prompt) and never reach a mission copilot. Also closed a real, separate gap found while writing this: `GetMissionStatus`'s disk-sample line (planned since step 2.6, deferred until step 5.1 landed) was never actually added — added it now (`copilot-tools.ts`, via the same `latestResourceSample` used by the tick and the report), since the new skills' own content references checking it. 7 new structural tests (`copilot-skills.unit.test.ts`: frontmatter validity + team/platform scope discovery for all four new skills, both extended team skills, and `incident-triage`) plus 2 new integration tests for the `GetMissionStatus` disk line (no-sample and with-sample cases) — **pending network**, Atlas still unreachable in this environment. `MAGI_V3_SPEC.md` gets a new "Resource oversight (ADR-0032)" paragraph (all nine `AnomalyCategory` values, the `atlas-storage-high` no-mission exception, the daily report, the four new + two extended skills) and a one-line fix adding the pre-existing `incident-triage` skill to its platform-skills list, noticed while editing the same line. **Per the plan's own instruction, all skill/prompt wording above is a first draft for your review, not fixed by any test** |
| 6.3 LIVE | ✅ 2026-09-22, `RESOURCE_REPORT_HOUR_UTC` set on `magi-control-dev` via `flyctl secrets set` (operator-run). Confirmed against real accumulated data for both users with missions on the deployment (checked via `flyctl ssh console` + a one-off `mongosh`-equivalent Node script, since Atlas was unreachable from the local dev sandbox but not from Fly's own network — the earlier "pending network" notes across 5.1/5.2/6.1 were a local-sandbox DNS issue, not a real Atlas outage): the report built correctly (per-mission spend/machine/upgrade/disk data all matched real history), `resourceSnapshots` claimed exactly once per user despite dozens of 5-min ticks since (idempotency holds — no duplicate mailbox posts), and one flag appeared organically (not seeded): `zz-lifecycle-smoke-f5nlew`'s 24h cumulative upgrade cap, genuinely exhausted from earlier session testing, computed correctly (23.9h used). The admin user's copilot woke via the waker, read the report, and replied with a substantive, accurate digest correctly referencing both flags and recommending concrete next steps — full pipeline proof, including real LLM judgment consuming the report format as intended. **One real finding, not a defect in this work**: the second user's copilot marked the report `readBy: ["copilot"]` but never replied — its turn was almost certainly interrupted by a control-plane redeploy that landed moments after the message arrived (unrelated push during this same session). This is a live hit of the already-documented gap **G-2** ("inbox messages marked-read before agent completes") in `docs/operational-resilience.md`, pre-existing and out of this step's scope to fix; since the message is already marked read it will not self-retry — left as-is rather than manually manipulating live mailbox state to force a re-run. **One wording observation for review, not a bug**: the admin copilot's reply was a full multi-section markdown digest, not the "≤120 words, at most one next step" the system prompt asks for — the LLM judged the two flags warranted more detail; worth deciding whether to tighten that line or accept the judgment call. Second-trigger no-op and report-arrival criteria both confirmed; digest-arrival confirmed for one of two users, with the other's non-reply attributed to G-2 rather than left unexplained |
...
| 5.3 | **OOM detection**: `fly-events.ts` (`classifyExit(event)`; `listMachineEvents()` one list call; dedupe on `exited_at`) → `oom-suspected` | `control-plane/src/fly-events.ts` | Unit fixtures taken from the real dev events captured 2026-09-20 (requested stop; `exit_code: 1` restarting) plus synthetic 137 / signal 9 / `oom_killed`; a requested stop is never OOM; the same exit is not reported twice. **LIVE:** none — 2.7's OOM was a clean Python `MemoryError`, never a Fly-level kill, so this never actually saw a real OOM exit event; not live-verified |
