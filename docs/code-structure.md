...
|---|---|---|---|---|---|
| `packages/agent-runtime-worker/src/monitor-server.ts` | 1876 | HTTP routing + auth + SSE + raw Mongo queries + file/git operations, almost entirely concentrated in one method: `handleRequest` (~725 lines, ~34 routes as a single if/else chain) | High | Planned — Sprint 28a | [#32](https://github.com/arnadu/magi_v3/issues/32) |
| `packages/agent-runtime-worker/src/daemon.ts` | 1455 | Full process bootstrap in `main()` (~745 lines): env parsing, Mongo/repo construction, workspace provisioning, signal handling, monitor/tool-server startup, job-runner startup, mailbox watching, orchestration launch — ~30 closure-captured locals threaded through sequentially | High | Planned — Sprint 28a | [#33](https://github.com/arnadu/magi_v3/issues/33) |
